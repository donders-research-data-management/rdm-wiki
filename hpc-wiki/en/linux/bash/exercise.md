...
In this exercise we will be extending script.sh by adding some BASH flow control constructions. We will be working with `if` and `for`. The `if`-statement and the `for`-loop are probably the most common flow control tools in the BASH hackers toolbox. Curious readers can look to the extra exercises and bonus materials for more advanced BASH scripting info. 
